...
function bindEvents() {
  ...
  $('toggleKeySectionBtn').onclick = () => toggleSetupSection('key');
  $('toggleStandardSectionBtn').onclick = () => toggleSetupSection('standard');
...
async function saveKey() {
  ...
  try {
    ...
    show('keySuccess', '接口密钥已保存到当前电脑本地。基础设置完成后可点击“收起设置”减少页面占用。');
...
async function saveStrictnessChoice() {
  ...
  try {
    ...
    updateStrictnessInfo();
    show('keySuccess', `严格度已保存为 ${STRICTNESS_TEXT[level]?.label || level + '度'}。基础设置完成后可点击“收起设置”。`);
  } catch (err) {
...
async function saveModelChoice() {
  ...
  try {
    ...
    updateModelInfo();
    show('keySuccess', '模型选择已保存。基础设置完成后可点击“收起设置”减少页面占用。');
  } catch (err) {
    show('keyError', err.message || String(err));
  }
}
...
async function saveStandard() {
  ...
  setStatus('standardStatus', '当前项目的岗位标准已保存。可点击“收起设置”减少页面占用。');
}
